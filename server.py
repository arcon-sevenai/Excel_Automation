...
# # (updated: short filenames + nested download paths)
# # ============================================================
...
# # Base URL for link generation (defaults to localhost for local testing)
# BASE_URL = os.getenv("BASE_URL", "http://localhost:8001")

# def _to_relpath(full_path: str) -> str:
#     """
#     Convert an absolute or project-relative path to a path relative to UPLOAD_DIR.
#     Example:
#       full_path = 'uploads\\manual_upload\\file\\sheet1\\processed.xlsx'
#       -> 'manual_upload/file/sheet1/processed.xlsx'
#     """
...
#     else:
#         # If it's already a rel path, normalize to POSIX-ish for URLs
#         rel = full_path
#     return rel.replace("\\", "/")
...
#         return JSONResponse(
#             {"status": "error", "message": f"Pinecone upload failed: {e}"},
#             status_code=500,
#         )
...
#     Uses process_manual_file() which:
...
#         # Full manual pipeline: returns a list of tuples (gpt_path, merged_path, missing_path)
#         results = process_manual_file(upload_path)
...
#             # Results now live in per-sheet subfolders with short names
#             processed_rel = _to_relpath(gpt_path) if gpt_path else None
#             merged_rel = _to_relpath(merged_path) if merged_path else None
#             missing_rel = _to_relpath(missing_path) if missing_path else None

#             sheets_info.append(
#                 {
#                     "processed_results": f"{BASE_URL}/download/{processed_rel}" if processed_rel else None,
#                     "merged_results": f"{BASE_URL}/download/{merged_rel}" if merged_rel else None,
#                     "missing_summary": f"{BASE_URL}/download/{missing_rel}" if missing_rel else None,
#                 }
#             )
...
#     """
#     try:
#         right_sheet_files = fetch_excel_from_jira(ticket_id)

#         if not right_sheet_files:
#             return JSONResponse(
#                 {
#                     "status": "error",
#                     "message": f"No valid sheets found for ticket {ticket_id}. "
#                                "Either there were no Excel/CSV attachments, or all sheets failed validation "
#                                "(images-only / no header+data / unreadable).",
#                 },
#                 status_code=400,
#             )
...
#             try:
#                 # Clean the per-sheet file then analyze (both now write short names in per-sheet folder)
#                 cleaned = create_clean_copy(sheet_path)
#                 working = cleaned if os.path.exists(cleaned) else sheet_path
#                 gpt_path, merged_path, missing_path = analyze_clean_sheet_file(working)

#                 analyzed_files.append(
#                     {
#                         "input_file": os.path.basename(sheet_path),
#                         "processed_results": f"{BASE_URL}/download/{_to_relpath(gpt_path)}" if gpt_path else None,
#                         "merged_results": f"{BASE_URL}/download/{_to_relpath(merged_path)}" if merged_path else None,
#                         "missing_summary": f"{BASE_URL}/download/{_to_relpath(missing_path)}" if missing_path else None,
#                     }
#                 )
#             except Exception as e:
#                 errors.append(
...
#         if analyzed_files and not errors:
#             return JSONResponse(
#                 {
#                     "status": "success",
...
#                 status_code=200,
#             )
#         elif analyzed_files and errors:
#             # Partial success
#             return JSONResponse(
#                 {
#                     "status": "partial_success",
#                     "message": f"Completed with some errors for ticket {ticket_id}.",
#                     "files": analyzed_files,
#                     "errors": errors,
#                 },
...
#     except Exception as e:
#         return JSONResponse(
#             {"status": "error", "message": f"Fetch + analyze failed: {e}"},
#             status_code=500,
#         )

# # ============================================================
# # ROUTE 4: DOWNLOAD FILES (Safe nested paths)
# # ============================================================
# @app.get("/download/{relpath:path}")
# async def download_file(relpath: str):
#     """
#     Serve files under uploads/ using a *relative* path (e.g., 'manual_upload/MyFile/sheet1/processed.xlsx').

#     - Prevents directory traversal.
#     - Works with short filenames because the path disambiguates the file.
#     """
#     # Normalize and block traversal
#     relpath_norm = os.path.normpath(relpath).lstrip("\\/")

#     # Reject attempts to escape uploads
#     if relpath_norm.startswith(".."):
#         raise HTTPException(status_code=400, detail="Invalid path.")

#     full_path = os.path.join(UPLOAD_DIR, relpath_norm)
#     full_path = os.path.normpath(full_path)

#     # Ensure the resolved path is still under UPLOAD_DIR
#     uploads_abs = os.path.abspath(UPLOAD_DIR)
#     full_abs = os.path.abspath(full_path)
#     if not full_abs.startswith(uploads_abs):
#         raise HTTPException(status_code=400, detail="Invalid path.")

#     if not os.path.exists(full_path) or not os.path.isfile(full_path):
#         return JSONResponse({"status": "error", "message": "File not found."}, status_code=404)

#     # Download with the leaf filename
#     return FileResponse(full_path, filename=os.path.basename(full_path), status_code=200)
